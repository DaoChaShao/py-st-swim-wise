...
from streamlit import data_editor, multiselect, markdown, columns, metric
from utils import BASE_CONFIG, load_csv


def main() -> None:
    """ Main Function """
    swimmer: DataFrame = load_csv(BASE_CONFIG.FILE_PATHS.DATA, dis_summary=True)
    dup_rows: int = swimmer.duplicated().sum()
    miss_values: int = swimmer.isnull().sum().sum()

    # Display Columns: 19
    COLS: list = swimmer.columns.tolist()
    ...
    with right:
        metric("Total Rows", len(swimmer), help="Total number of rows in the dataset")
    ...
    data_editor(swimmer[cols], height=500, width="stretch", hide_index=True, disabled=True, placeholder="Data Preview")
